...
class MRIDataset(torch.utils.data.Dataset):
    ...
    def intensity_normalize(self, volume):
       ...
       # Assuming 'volume' is a PyTorch tensor with shape [X, Y, Z, C]
        volume_mean = torch.mean(volume, dim=(0, 1, 2), keepdim=True)  # Mean over X, Y, Z, keep C intact
        volume_std = torch.std(volume, dim=(0, 1, 2), keepdim=True)    # Std over X, Y, Z, keep C intact

        epsilon = torch.finfo(volume_std.dtype).eps  # Small positive number for the data type
