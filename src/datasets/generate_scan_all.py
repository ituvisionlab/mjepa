...
MASTER_CSV = "/gpfs/home/unalg01/jepa/src/datasets/SCAN_NIFTI_all_with_betmask_and_bbox.csv"
LOG_FILE = "/gpfs/home/unalg01/jepa/src/datasets/scan_dataset_summary.log"
...
def passes_fov_spacing_filter(img, file_path):
    try:
        data = img.get_fdata()
        if data.ndim != 3:
            print(f"[SKIP] Non-3D volume: {file_path} (dim={data.ndim})")
            return False
        ...
        if any(f < MIN_FOV for f in fov) or any(s > MAX_SPACING for s in spacing):
            print(f"[SKIP] FOV/spacing filtered: {file_path}")
            return False
        return True
    except Exception as e:
...
for subject_id in os.listdir(DATA_ROOT):
    ...
    for fname in os.listdir(subj_path):
        ...
        if not any(key.lower() in file_lower for key in VALID_CONTRASTS):
...
with open(LOG_FILE, "w") as f:
    ...
    f.write(f"Included subject IDs: {sorted(list(included_subjects))}\n")
    f.write(f"Contrast filter used: {VALID_CONTRASTS}\n")
